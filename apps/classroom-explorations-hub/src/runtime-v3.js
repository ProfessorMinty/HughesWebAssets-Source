(function () {
  ...
  function normalizeYouTube(sourceUrl) {
    ...
    if (!/^[A-Za-z0-9_-]{11}$/.test(id)) {
      throw new HubRuntimeError(
        "HUB_EXTERNAL_MEDIA_UNSUPPORTED",
  ...
  function project(source, registry) {
    if (
      ...
      (source.page && source.page.type !== "classroom-explorations-hub")
    ) {
      throw new HubRuntimeError(
        "HUB_SCHEMA_UNSUPPORTED",
        "Unsupported Hub authoring source."
      );
    }
    ...
    if (
      ...
      throw new HubRuntimeError(
        "HUB_ROUTE_SCHEMA_UNSUPPORTED",
        "Unsupported Hub route registry."
      );
    }

    var routes = new Map(
      registry.routes.map(function (route) {
        return [route.ref, route];
      })
    );
    var years = new Map(
      source.data.schoolYears.map(function (year) {
        return [year.id, year];
      })
    );
    var explorations = new Map(
      source.data.explorations.map(function (item) {
        return [item.id, item];
      })
    );
    var twwl = new Map(
      source.data.twwl.map(function (item) {
        return [item.id, item];
      })
    );
    var media = new Map(
      source.data.media.map(function (item) {
        return [item.id, item];
      })
    );
    ...
    function resolveImage(image) {
      ...
      if (image.kind !== "external-url" || !image.url) {
        throw new HubRuntimeError(
          ...
          "The review runtime requires a resolved image URL."
        );
      }
    ...
    function resolveExploration(item, includeYear) {
      ...
      var resolved = {
        ...
        learningPoints: Array.isArray(item.learningPoints)
          ? item.learningPoints.slice()
          : [],
      ...
      if (includeYear) {
        ...
        resolved.schoolYearLabel = years.has(item.schoolYear)
          ? years.get(item.schoolYear).label
          : item.schoolYear;
      }
    ...
    function resolveTwwl(item) {
      ...
      return {
        ...
        schoolYearLabel: years.has(item.schoolYear)
          ? years.get(item.schoolYear).label
          : item.schoolYear,
        title: item.title,
        summary: item.summary,
    ...
    var currentExploration = explorations.get(
      composition.currentExplorationId
    );
    if (!currentExploration) {
      throw new HubRuntimeError(
        "HUB_CURRENT_EXPLORATION_UNKNOWN",
        "Current Exploration does not exist."
      );
    }
    ...
    if (!featured) {
      throw new HubRuntimeError(
        "HUB_FEATURED_VIDEO_UNKNOWN",
        "Featured media does not exist."
      );
    }

    var currentTwwl =
      composition.currentTwwl.state === "published"
        ? {
            id: composition.currentTwwl.id,
            state: "published",
            content: resolveTwwl(
              twwl.get(composition.currentTwwl.contentId)
            )
          }
        : {
            id: composition.currentTwwl.id,
    ...
    return {
      ...
      archives: composition.previousYears.map(function (archive) {
        return {
          ...
          label: years.has(archive.schoolYear)
            ? years.get(archive.schoolYear).label
            : archive.schoolYear,
          state: archive.state,
  ...
  function imageFigure(item, className) {
    ...
    var fallback = create(
      "div",
      "hub-v2-image-fallback",
      subjectEmoji(item.id)
    );
  ...
  function compass() {
    ...
    wrap.innerHTML = [
      ...
      '<defs>',
      '<radialGradient id="hrvV3CompassGlow" cx="50%" cy="50%" r="50%"><stop offset="0" stop-color="#fff3b2" stop-opacity=".98"/><stop offset=".38" stop-color="#68e1c2" stop-opacity=".38"/><stop offset="1" stop-color="#68e1c2" stop-opacity="0"/></radialGradient>',
      '<linearGradient id="hrvV3CompassRing" x1="0" x2="1"><stop offset="0" stop-color="#6ee7ce"/><stop offset=".48" stop-color="#ffe69a"/><stop offset="1" stop-color="#9cb8ff"/></linearGradient>',
      '</defs>',
      '<circle cx="90" cy="90" r="87" fill="url(#hrvV3CompassGlow)"/>',
      '<circle class="hub-v2-compass-orbit" cx="90" cy="90" r="66" fill="rgba(5,14,35,.78)" stroke="url(#hrvV3CompassRing)" stroke-width="4"/>',
      ...
      '<circle cx="90" cy="90" r="3" fill="#25457d"/>',
      '</svg>'
    ].join("");
    return wrap;
  }
  ...
  Controller.prototype.start = function () {
    this.root.classList.add("hrv-hub-v2", "hrv-hub-v3");
    this.root.setAttribute("data-hrv-page-id", this.manifest.page.id);
    this.root.setAttribute(
      "data-hrv-source-ref",
      this.options.sourceRef || "review"
    );
    ...
    document.documentElement.classList.add(
      "hrv-page-classroom-explorations-ready"
    );

    window.dispatchEvent(
      new CustomEvent("hrv:page-ready", {
        detail: {
  ...
  Controller.prototype.destroy = function () {
    ...
    this.destroyed = true;
    this.listeners.splice(0).forEach(function (off) {
      off();
    });
    this.observers.splice(0).forEach(function (observer) {
      observer.disconnect();
    });
    this.root.replaceChildren();
    this.root.classList.remove("hrv-hub-v2", "hrv-hub-v3");
  ...
  Controller.prototype.render = function () {
    var skip = anchor(
      "Skip to the Current Exploration",
      "#hrv-v2-current",
      "hub-v2-skip"
    );
    var main = create("main", "hub-v2-museum");
    main.id = "hrv-v2-museum";
    main.setAttribute("data-hrv-auto-scroll-target", "");
  ...
  Controller.prototype.hero = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-hero"),
      "data-hrv-node-id",
      copy.nodeId
    );
    section.setAttribute("data-reveal", "");

    var shell = create("div", "hub-v2-shell hub-v3-shell-wide");
    ...
    museumLabel.append(
      create("span", "hub-v2-museum-label-icon", "✦"),
      create("span", "", "Ms Hughes’ Classroom Museum")
    );

    var year = create(
      "span",
      "hub-v2-year-chip hub-v3-year-chip",
      this.manifest.page.schoolYearLabel + " Museum"
    );
    ...
    copy.pillars.forEach(function (pillar, index) {
      var item = create("li");
      item.append(
        create("span", "hub-v2-pillar-number", String(index + 1)),
        create("span", "", pillar)
      );
      pillars.appendChild(item);
    });
    left.append(pillars, year);
    ...
    oath.append(
      create("strong", "", copy.oathTitle),
      create("p", "", copy.oathBody)
    );
  ...
  Controller.prototype.map = function () {
    ...
    var shell = create(
      "div",
      "hub-v2-shell hub-v2-map-shell hub-v3-shell-wide"
    );
  ...
  Controller.prototype.welcome = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-welcome"),
      "data-hrv-node-id",
      copy.nodeId
    );
    ...
    var shell = create("div", "hub-v2-shell hub-v3-shell-theater");
    var card = create("div", "hub-v2-panel hub-v2-theater");
    var header = sectionHeading(copy, "hrv-v2-welcome-title");
    header.appendChild(
      create("span", "hub-v2-room-badge", "Orientation Theater")
    );
  ...
  Controller.prototype.currentExploration = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-current"),
      "data-hrv-node-id",
      copy.nodeId
    );
    ...
    section.setAttribute(
      "data-hrv-slot-id",
      "hub-slot:current-exploration"
    );

    var shell = create("div", "hub-v2-shell hub-v3-shell-wide");
    shell.appendChild(sectionHeading(copy, "hrv-v2-current-title"));

    var card = setIdentity(
      create("article", "hub-v2-current-card"),
      "data-hrv-content-id",
      item.id
    );
    ...
    art.appendChild(
      create("span", "hub-v2-featured-ribbon", "Now Exploring")
    );
    ...
    body.append(
      create(
        "p",
        "hub-v2-specimen-label",
        "Featured Exhibit • " + this.manifest.page.schoolYearLabel
      ),
    ...
    var open = anchor(
      ...
      item.href,
      "hub-v2-action hub-v2-action-primary"
    );
  ...
  Controller.prototype.currentTwwl = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-twwl"),
      "data-hrv-node-id",
      copy.nodeId
    );
    ...
    body.append(
      create("p", "hub-v2-eyebrow", copy.eyebrow),
      create("h2", "hub-v2-section-title", copy.title)
    );
    ...
    if (slot.state === "published" && slot.content) {
      ...
      body.append(
        create("h3", "hub-v2-lantern-title", item.title),
        create("p", "hub-v2-section-summary", item.summary)
      );
      body.append(
        tagList(item.tags),
        anchor(
          "Read This Week’s Learning Story",
          item.href,
          "hub-v2-action hub-v2-action-primary"
        )
      );
  ...
  Controller.prototype.pastExplorations = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-past"),
      "data-hrv-node-id",
      copy.nodeId
    );
    ...
    items.forEach(function (item, index) {
      var card = setIdentity(
        create("article", "hub-v2-exploration-card"),
        "data-hrv-content-id",
        item.id
      );
      card.setAttribute("data-pointer-light", "");
  ...
  Controller.prototype.pastTwwl = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-memory-hall"),
      "data-hrv-node-id",
      copy.nodeId
    );
    section.setAttribute("data-reveal", "");
    ...
    items.forEach(function (item, index) {
      var card = setIdentity(
        create("article", "hub-v2-memory-card"),
        "data-hrv-content-id",
        item.id
      );
      ...
      body.append(
        create(
          "p",
          "hub-v2-memory-meta",
          item.schoolYearLabel + " • Learning Story"
        ),
      ...
      card.append(
        icon,
        body,
        anchor("Open Story", item.href, "hub-v2-memory-link")
      );
  ...
  Controller.prototype.archives = function () {
    ...
    var section = setIdentity(
      create("section", "hub-v2-section hub-v2-archives"),
      "data-hrv-node-id",
      copy.nodeId
    );
    ...
    body.append(
      create("p", "hub-v2-eyebrow", copy.eyebrow),
      create("h2", "hub-v2-section-title", copy.title),
      create("p", "hub-v2-section-summary", copy.intro)
    );

    var links = create("div", "hub-v2-archive-links");
    if (!archives.length) {
      links.appendChild(
        create("p", "hub-v2-empty", "The archive wing is being cataloged.")
      );
    } else {
      archives.forEach(function (archive) {
        if (archive.state === "published" && archive.href) {
          var open = anchor(
            "Open the " + archive.label + " Museum Wing",
            archive.href,
            "hub-v2-action hub-v2-action-primary"
          );
          ...
          var pending = create(
            "div",
            "hub-v2-archive-pending",
            archive.label + " archive coming soon"
          );
  ...
  Controller.prototype.footer = function () {
    ...
    var footer = setIdentity(
      create("footer", "hub-v2-footer"),
      "data-hrv-node-id",
      copy.nodeId
    );
    var shell = create("div", "hub-v2-shell hub-v2-footer-shell");
    shell.append(
      create("span", "hub-v2-footer-star", "✦"),
      create("p", "", copy.text),
      create("span", "hub-v2-footer-star", "✦")
    );
  ...
  Controller.prototype.wirePointerLight = function () {
    ...
    var self = this;
    this.root
      .querySelectorAll("[data-pointer-light]")
  ...
  function mount(root, options) {
    if (!(root instanceof Element)) {
      throw new HubRuntimeError(
        "HUB_MOUNT_INVALID",
        "A valid Hub mount element is required."
      );
    }
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("hrv:page-error", {
          detail: {
            pageId:
              options && options.source && options.source.page
                ? options.source.page.id
                : "hrv-page:classroom-explorations",
            code: error && error.code ? error.code : "HUB_RUNTIME_FAILED",
            message:
              error && error.message ? error.message : "Hub runtime failed."
          }
        })
      );
      throw error;
