...
class HubController {
  ...
  private renderMuseum(): void {
    ...
    museum.append(
      this.createSky(),
      this.createParade(),
      this.createHero(manifest, pastExplorations.length, pastTwwl.length, doorways.length),
    );
    if (video) museum.append(this.createVideo(video));
    if (currentExploration) museum.append(this.createCurrentExploration(currentExploration));
    if (currentTwwl) museum.append(this.createCurrentTwwl(currentTwwl));
    museum.append(this.createDivider(manifest));
    museum.append(this.createArchiveWing("exploration", pastExplorations));
    museum.append(this.createArchiveWing("twwl", pastTwwl));
  ...
  private createSky(): HTMLElement {
    ...
    sky.append(el("div", "layer stars"), el("div", "layer clouds"), el("div", "layer aurora"));
    return sky;
  }
  ...
  private createParade(): HTMLElement {
    ...
    ["🚀", "🧭", "🔬", "🧪", "🌍", "🦋"].forEach((icon) => parade.append(el("span", "pi", icon)));
    return parade;
  }
  ...
  private createVideo(record: HubRecord): HTMLElement {
    const section = el("section", "intro-video card");
  ...
  private createCurrentExploration(record: HubRecord): HTMLElement {
    const section = el("section", "current card reef-feature zinnia-feature");
    applyRecordState(section, record);
    ...
    visual.append(el("div", "visual-badge", record.presentation?.visualBadge ?? "Zinnia Greenhouse"), el("div", "ring one"), el("div", "ring two"), el("div", "spore sp1"), el("div", "spore sp2"), el("div", "spore sp3"));
  ...
  private createCurrentTwwl(record: HubRecord): HTMLElement {
    const section = el("section", "learn-now card lantern-feature");
    applyRecordState(section, record);
    ...
    visual.append(el("div", "visual-badge learn-badge-visual", record.presentation?.visualBadge ?? "Learning Lantern"), el("div", "lantern-orb", record.emoji), el("div", "ring one"), el("div", "ring two"), el("div", "sparkle sp1"), el("div", "sparkle sp2"), el("div", "sparkle sp3"));
    Array.from(visual.children).forEach((child) => child.setAttribute("aria-hidden", "true"));
    right.append(visual);
    section.append(left, right);
    return section;
  ...
  private createDivider(manifest: HubManifest): HTMLElement {
    const divider = el("div", "museum-divider");
  ...
  private createArchiveWing(type: "exploration" | "twwl", records: HubRecord[]): HTMLElement {
    ...
    const section = el("section", isExploration ? "past card exhibit-gallery" : "learn-archive card learning-gallery");
    ...
    if (records.length === 0) {
      section.append(el("p", "archive-empty", isExploration ? "Completed 2026–2027 explorations will gather in this gallery." : "Approved 2026–2027 learning recaps will gather in this gallery."));
      return section;
    }
  ...
  private createYearDoorways(records: HubRecord[]): HTMLElement {
    const section = el("section", "past card year-archive-gallery");
    section.setAttribute("aria-labelledby", "year-archive-heading");
    const top = el("div", "past-top");
    const copy = el("div");
