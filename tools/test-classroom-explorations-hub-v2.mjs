...
const sourceSchema = readJson(
  "schemas/classroom-explorations-hub.source.schema.json"
);
const runtimeSchema = readJson(
  "schemas/classroom-explorations-hub.runtime.schema.json"
);
const routeSchema = readJson("schemas/hrv-route-registry.schema.json");
const controlSchema = readJson(
...
const hubRoute = routes.routes.find(
  (item) => item.ref === "hrv-route:classroom-explorations"
);
assert.ok(hubRoute);
...
const expectedManagedChildren = [
  ...
  [
    "hrv-route:butterflies",
    2,
    674,
    "exploration-butterflies",
    "/hub/exploration-butterflies/",
    "exploration"
  ],
  [
    ...
    3,
    1518,
    "exploration-great-barrier-reef",
    "/hub/exploration-great-barrier-reef/",
    "exploration"
  ],
  [
    "hrv-route:mushrooms",
    4,
    413,
    "exploration-mushrooms",
    "/hub/exploration-mushrooms/",
    "exploration"
  ],
  [
    "hrv-route:owls",
  ...
  [
    "hrv-route:russian-winter",
    6,
    1043,
    "twwl-traditions-of-russian-winter",
    "/hub/twwl-traditions-of-russian-winter/",
    "twwl"
  ],
  [
    "hrv-route:tubers",
  ...
  [
    ...
    8,
    2463,
    "exploration-zinnia-page",
    "/hub/exploration-zinnia-page/",
    "exploration"
  ],
  [
    "hrv-route:archive-2025-2026",
    9,
    2627,
    "archive-2025-2026",
    "/hub/archive-2025-2026/",
    "archive"
  ]
];

for (const [ref, order, pageId, slug, routePath, kind] of
  expectedManagedChildren) {
...
assert.equal(
  runtime.page.href,
  "https://rmhughes.edublogs.org/hub/"
);
assert.equal(runtime.page.currentSchoolYear, "2026-2027");
assert.equal(
  runtime.current.exploration.id,
  "summer-bloom-adoption-project"
);
assert.equal(
  runtime.current.exploration.href,
  "https://rmhughes.edublogs.org/hub/exploration-zinnia-page/"
);
assert.equal(runtime.current.twwl.state, "coming-soon");
assert.equal(
  runtime.current.featuredMedia.embedUrl,
  "https://www.youtube-nocookie.com/embed/kRTJp4pqbtg"
);
assert.deepEqual(
...
assert.ok(
  !runtime.galleries.pastExplorations.some((item) =>
    item.id.includes("caterpillars-in-the-classroom-historical")
  )
);
assert.equal(runtime.archives.length, 1);
assert.equal(runtime.archives[0].state, "published");
assert.equal(
  runtime.archives[0].href,
  "https://rmhughes.edublogs.org/hub/archive-2025-2026/"
);

const copyNodeIds = Object.values(source.data.copy).map(
  (block) => block.nodeId
);
assert.equal(new Set(copyNodeIds).size, copyNodeIds.length);
assert.deepEqual(
  new Set(control.editor.nodes.map((node) => node.nodeId)),
  new Set(copyNodeIds)
);
...
const editResult = applyHubCommand({
  ...
  command: {
    ...
    payload: {
      ...
      patch: {
        invitation: "A deterministic Lanternworks edit test."
      }
    }
  }
});
...
assert.throws(
  ...
  (error) =>
    error instanceof HubTransactionError &&
    error.code === "HUB_STABLE_ID_IMMUTABLE"
);
...
assert.throws(
    ...
    applyHubCommand({
      ...
      command: {
        ...
        payload: {
          ...
          patch: {
            text: "This stale command must never apply."
          }
        }
      }
    }),
  (error) =>
    error instanceof HubTransactionError &&
    error.code === "HUB_TRANSACTION_STALE"
);
...
assert.equal(
  swapResult.documents.source.data.composition.currentExplorationId,
  swapExploration.id
);
assert.ok(
  swapResult.documents.source.data.composition.pastExplorationIds.includes(
    "summer-bloom-adoption-project"
  )
);
assert.ok(
...
assert.ok(
  swapResult.documents.routes.routes.some(
    (route) => route.ref === swapRoute.ref
  )
);
validateHubSemantics(
  swapResult.documents.source,
  swapResult.documents.routes
);

assert.throws(
  () => setCurrentExploration(source, routes, "mushrooms"),
  (error) =>
    error instanceof HubContractError &&
    error.code === "HUB_CURRENT_EXPLORATION_YEAR_MISMATCH"
);

const restoredCurrent = setCurrentExploration(
  swapResult.documents.source,
  swapResult.documents.routes,
  "summer-bloom-adoption-project"
);
assert.equal(
  restoredCurrent.data.composition.currentExplorationId,
  "summer-bloom-adoption-project"
);
assert.ok(
  restoredCurrent.data.composition.pastExplorationIds.includes(
    swapExploration.id
  )
);
assert.ok(
...
const reordered = reorderPastExplorations(source, routes, [
  "great-barrier-reef",
  "butterflies-in-the-classroom",
  "mushrooms"
]);
assert.deepEqual(
  reordered.data.composition.pastExplorationIds,
  [
    "great-barrier-reef",
    "butterflies-in-the-classroom",
    "mushrooms"
...
const publishedTwwl = setCurrentTwwl(
  fixtureCurrentTwwlSource,
  routes,
  "test-current-learning-story"
);
assert.equal(
  publishedTwwl.data.composition.currentTwwl.state,
  "published"
);
assert.equal(
  publishedTwwl.data.composition.currentTwwl.contentId,
  "test-current-learning-story"
);

assert.throws(
  () =>
    updateExploration(source, routes, "mushrooms", {
      id: "renamed"
    }),
...
assert.throws(
  () =>
    updateRoute(routes, "hrv-route:mushrooms", {
      wordpressPageId: 999
    }),
...
assert.throws(
  () => {
    ...
    invalid.data.composition.pastExplorationIds.push(
      invalid.data.composition.currentExplorationId
    );
    validateHubSemantics(invalid, routes);
  },
  (error) =>
    error instanceof HubContractError &&
    error.code === "HUB_CONTENT_PLACED_MULTIPLE_TIMES"
);

console.log(
  "[hub v2] authoring, routes, runtime projection, archive, and corrected gallery passed"
);
console.log(
  "[hub v2] stale rejection, stable IDs, current-year enforcement, node edit, and atomic Hub swap passed"
);
