...
if (!/^\d{4}\.\d{2}\.\d{2}\.\d+$/.test(runtimeVersion || "")) {
  throw new Error("runtimeVersion must be YYYY.MM.DD.N");
}
if (!/^pub-[0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{3}$/.test(publicationId || "")) {
  throw new Error("publicationId must be pub-YYYY-MM-DD-NNN");
}
if (!/^[a-f0-9]{40}$/.test(sourceRevision || "")) {
  throw new Error("sourceRevision must be a 40-character Git commit SHA");
}
if (
  previousPublicationId !== "none" &&
  !/^pub-[0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{3}$/.test(previousPublicationId)
) {
  throw new Error("previousPublicationId must be none or pub-YYYY-MM-DD-NNN");
}
...
const dist = resolve(
  process.env.HRV_ARCHIVE_DIST_ROOT || resolve(root, "dist/classroom-explorations-archive")
);
...
const publicationTarget = resolve(publicationParent, publicationId);
...
const contentHash = snapshot.canonicalSha256;
const contentTarget = resolve(contentParent, contentHash);
...
if (
  ...
  incomingContent.page?.id !== PAGE_ID ||
  incomingContent.page?.type !== PAGE_TYPE
) {
...
if (await exists(publicationTarget)) {
  throw new Error(`Immutable publication already exists: ${publicationTarget}`);
}
await Promise.all([
...
try {
  ...
  if (contentExists) {
    ...
    contentTemp = await mkdtemp(resolve(contentParent, `.${contentHash}.tmp-`));
  ...
  const publication = {
    ...
    runtime: {
      ...
      bootstrap: {
        path: `../../runtime/${runtimeVersion}/bootstrap.js`,
        sha256: await digest(resolve(runtimeCandidate, "bootstrap.js"))
      },
      script: {
        path: `../../runtime/${runtimeVersion}/runtime.js`,
        sha256: await digest(resolve(runtimeCandidate, "runtime.js"))
      },
      style: {
        path: `../../runtime/${runtimeVersion}/archive.css`,
        sha256: await digest(resolve(runtimeCandidate, "archive.css"))
      },
      hostCompat: {
        path: `../../runtime/${runtimeVersion}/host-compat.css`,
        sha256: await digest(resolve(runtimeCandidate, "host-compat.css"))
      },
    ...
    content: {
      ...
      runtimeSchemaVersion: "1.0",
      manifest: {
        path: `../../content/${contentHash}/manifest.json`,
  ...
  if (
    ...
    process.env.HRV_ARCHIVE_TEST_FAIL_BEFORE_PUBLICATION_PROMOTION === "true"
  ) {
    throw new Error("Injected failure before archive publication promotion.");
  }
  await rename(publicationTemp, publicationTarget);
  publicationTemp = null;
  promotedPublication = true;

  console.log(
    `[archive] staged immutable publication ${publicationId}; previous known good = ${publication.previousKnownGoodPublication ?? "none (new baseline)"}`
  );
} catch (error) {
  const cleanupErrors = [];
  ...
  for (const [path, promoted] of [
    [publicationTarget, promotedPublication],
  ...
  if (cleanupErrors.length) {
    throw new AggregateError(
      [error, ...cleanupErrors],
      "Immutable archive staging failed and cleanup was incomplete."
    );
  }
  throw error;
}
