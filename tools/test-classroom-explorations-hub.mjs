...
assert.ok(hubRoute, "Hub route must exist");
...
for (const [ref, order, pageId, slug, routePath, kind] of expectedManagedChildren) {
  ...
  assert.equal(route.wordpressParentPageId, 17, `${ref} must remain beneath page 17`);
  assert.equal(route.wordpressMenuOrder, order, `${ref} menu order drifted`);
  assert.equal(route.wordpressPageId, pageId, `${ref} WordPress identity drifted`);
  assert.equal(route.slug, slug, `${ref} slug drifted`);
  assert.equal(route.path, routePath, `${ref} path drifted`);
  assert.equal(route.kind, kind, `${ref} kind drifted`);
}

const managedChildRefs = new Set(
  routes.routes
    .filter((item) => item.wordpressParentPageId === 17)
    .map((item) => item.ref)
);
assert.deepEqual(managedChildRefs, new Set(expectedManagedChildren.map(([ref]) => ref)));
...
assert.equal(
  runtime.galleries.pastExplorations.at(-1).href,
  "https://rmhughes.edublogs.org/hub/exploration-great-barrier-reef/"
);
assert.ok(
  !runtime.galleries.pastExplorations.some((item) => item.id === "caterpillars-in-the-classroom-historical"),
  "The visible gallery must use the corrected Butterflies destination"
);
...
assert.equal(runtime.archives[0].schoolYear, "2025-2026");
...
assert.ok(
  swapResult.documents.source.data.composition.pastExplorationIds.includes("summer-bloom-adoption-project")
);
assert.ok(
  !swapResult.documents.source.data.composition.pastExplorationIds.includes(swapExploration.id)
);
...
console.log("[hub contract] authoring, routes, runtime projection, archive, and corrected gallery passed");
console.log("[hub transactions] stale rejection, stable IDs, node edit, and atomic Hub swap passed");
