(() => {
  ...
  const fallbackClassName = root?.className || "";
  const fail = (message, error) => {
    if (root) {
      ...
      const status = root.querySelector("[data-hrv-fallback-status]");
      if (status) status.textContent = message;
    }
    console.error("[HRV Classroom Explorations]", message, error || "");
    window.dispatchEvent(new CustomEvent("hrv:page-error", { detail: { pageId: "hrv-page:classroom-explorations", message } }));
  };
  if (!root) return;
  ...
  try { publicationResolved = new URL(publicationUrl, window.location.href); } catch { publicationResolved = null; }
  const localPreview = publicationResolved && ["localhost", "127.0.0.1"].includes(publicationResolved.hostname);
  if (!publicationResolved || (publicationResolved.protocol !== "https:" && !localPreview)) {
    fail("The classroom museum is temporarily unavailable.", new Error("Missing secure immutable publication URL."));
    return;
  }
  ...
  (async () => {
    try {
      const publicationResponse = await fetch(publicationUrl, { credentials: "omit", cache: "no-store" });
      ...
      try {
        ...
        if (typeof module.mountClassroomExplorationsHub !== "function") throw new Error("Renderer mount export missing.");
        module.mountClassroomExplorationsHub(root, manifest);
      } finally {
        URL.revokeObjectURL(moduleUrl);
      }
    } catch (error) {
      document.querySelectorAll("style[data-hrv-classroom-hub-style]").forEach((el) => el.remove());
      fail("The classroom museum is temporarily unavailable. Please try again soon.", error);
    }
  })();
})();
